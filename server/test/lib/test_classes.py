...
class ClassesTest(TestCase):
    def test_env_parser(self):
        ...
        self.assertEqual(
            env_parser.get_arg_from_environ("TEST_VALUE"),
            [1, 2, 3, 4]
        )
        ...
        res = env_parser.get_arg_from_file(
            "value_2",
            file.name,
        ...
        self.assertEqual(res, "raw_value")
    ...
    def test_hasher(self):
        ...
        self.assertEqual(
            Hasher.hash_algs,
            [shake_24, Hasher.supported_algorithms["sha3_256"]]
        )
        ...
        self.assertEqual(
            Hasher.hash_algs,
            [
                Hasher.supported_algorithms["sha3_384"],
                Hasher.supported_algorithms["blake2b"],
            ]
        )
    ...
    def test_bstring_cycle(self):
        ...
        self.assertEqual(
            b"".join(b for (b, _) in zip(cycle, range(10))),
            b"1231231231"
        )
        self.assertEqual(
            b"".join(b for (b, _) in zip(cycle, range(10))),
